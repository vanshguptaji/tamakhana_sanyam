...
const Add = ({ token, editProduct, setEditProduct }) => {
  ...
  const [image4,setImage4] = useState(false)
   ...
   const [category, setCategory] = useState("Light Weighted");
   const [subCategory, setSubCategory] = useState("Topwear");
   const [bestseller, setBestseller] = useState(false);
   const [sizes, setSizes] = useState([]);

   // Add useEffect to populate fields when editing
   React.useEffect(() => {
    if (editProduct) {
      setName(editProduct.name || "");
      setDescription(editProduct.description || "");
      setPrice(editProduct.price || "");
      setCategory(editProduct.category || "Light Weighted");
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
        ...
        <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
            ...
            <div>
              ...
              <select onChange={(e) => setCategory(e.target.value)} className='w-full px-3 py-2'>
                  <option value="Light Weighted">Light Weighted</option>
                  <option value="Dry Fruits">Dry Fruits</option>
                  <option value="Healthy Makhana">Healthy Makhana</option>
              </select>
            </div>
        ...
        {editProduct && (
          <button
            type="button"
            className="w-28 py-3 mt-2 bg-gray-300 text-black"
